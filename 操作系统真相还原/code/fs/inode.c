...
{ // io_buf是用于硬盘io的缓冲区
    ...
    {                                                             // 若是跨了两个扇区,就要读出两个扇区再写入两个扇区
        ...
        ide_read(part->my_disk,  inode_buf,inode_pos.sec_lba, 2); // inode在format中写入硬盘时是连续写入的,所以读入2块扇区

        /* 开始将待写入的inode拼入到这2个扇区中的相应位置 */
        memcpy((inode_buf + inode_pos.off_size), &pure_inode, sizeof(struct inode));

        /* 将拼接好的数据再写入磁盘 */
        ide_write(part->my_disk,  inode_buf, inode_pos.sec_lba,2);
    }
    else
    { // 若只是一个扇区
        ide_read(part->my_disk, inode_buf, inode_pos.sec_lba, 1);
        memcpy((inode_buf + inode_pos.off_size), &pure_inode, sizeof(struct inode));
        ide_write(part->my_disk, inode_buf, inode_pos.sec_lba, 1);
    }
}
