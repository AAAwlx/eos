...
struct mem_pool kernel_pool, user_pool; // 为kernel与user分别建立物理内存池，让用户进程只能从user内存池获得新的内存空间，
...
static void mem_pool_init(uint32_t all_mem)
{
    ...
    kernel_pool.pool_bitmap.bitmap_len = kbm_length; // 赋值给管理内核使用的物理内存池的位图长度
    user_pool.pool_bitmap.bitmap_len = ubm_length;   // 赋值给管理用户使用的物理内存池的位图长度
    ...
    kernel_vaddr.vaddr_bitmap.bitmap_len = kbm_length; // 赋值给管理内核可以动态使用的虚拟地址池（堆区）的位图长度，
...
static void *vaddr_get(enum pool_flags pf, uint32_t pg_cnt)
{
    ...
    { // 用户内存池
        ...
        bit_idx_start = bitmap_scan(&cur->userprog_vaddar.vaddr_bitmap, pg_cnt);
        if (bit_idx_start == -1)
        {
            return NULL;
        }
        while (cnt < pg_cnt)
        {
            bitmap_set(&cur->userprog_vaddar.vaddr_bitmap, bit_idx_start + cnt++, 1);
        }
...
static void *palloc(struct mem_pool *m_pool)
{
    /* 扫描或设置位图要保证原子操作 */
...
void *sys_malloc(uint32_t size)
{
    ...
    if (size<0||size>pool_size)//如果大于内存池或小于0
    {
        return NULL;
    }
    struct arena *a;
