...
static void select_sector(struct disk* hd, uint32_t lba, uint8_t sec_cnt) {
    ASSERT(lba <= max_lba);
    struct ide_channel* channel = hd->mychannel;
    ...
    outb(reg_lba_l(channel),
         lba);  // lba地址的低8位,不用单独取出低8位.outb函数中的汇编指令outb
                // %b0, %w1会只用al。
...
static void read_from_sector(struct disk* hd, void* buf, uint8_t sec_cnt) {
    ...
    inw(reg_data(hd->mychannel), buf, size_in_byte / 2);
}
...
static void write2sector(struct disk* hd, void* buf, uint8_t sec_cnt) {
    ...
    outw(reg_data(hd->mychannel), buf, size_in_byte / 2);
}
...
/* 从硬盘读取sec_cnt个扇区到buf */
void ide_read(struct disk* hd,void* buf,uint32_t lba,uint32_t sec_cnt) {  // 此处的sec_cnt为32位大小
    ASSERT(lba <= max_lba);
    ASSERT(sec_cnt > 0);
    lock_acquire(&hd->mychannel->c_lock);  // 一个通道同时只允许操作一个磁盘
    /*1.选择操作磁盘*/
    select_disk(hd);
    ...
    while (secs_done < sec_cnt) {
        ...
        /*3.执行的命令写入reg_cmd寄存器*/
        cmd_out(hd->mychannel, CMD_READ_SECTOR);
...
void ide_write(struct disk* hd, char* buf, uint32_t lba, uint32_t sec_cnt) {
    ASSERT(lba <= max_lba);
    ASSERT(sec_cnt > 0);
    ...
    while (secs_done < sec_cnt) {
        ...
        select_sector(
            hd, lba + secs_done,
            secs_op);  // 先将待读的块号lba地址和待读入的扇区数写入lba寄存器

        /* 3 执行的命令写入reg_cmd寄存器 */
        cmd_out(hd->mychannel, CMD_WRITE_SECTOR);  // 准备开始写数据
...
static void identify_disk(struct disk* hd) {
    char id_info[512];
    select_disk(hd);
    cmd_out(hd->mychannel, CMD_IDENTIFY);
    /* 向硬盘发送指令后便通过信号量阻塞自己,
...
static void partition_scan(struct disk* hd, uint32_t ext_lba) {
    ...
    printk("bs:%x\n", bs);
    ide_read(hd, bs, ext_lba, 1);
    printk("partition_scan");
    uint8_t part_idx = 0;
...
void ide_init() {
    ...
    channel_cnt = DIV_ROUND_UP(
        hd_cnt, 2);  // 一个ide通道上有两个硬盘,根据硬盘数量反推有几个ide通道
    ...
    while (channel_no < channel_cnt) {
        ...
        switch (channel_no) {
            case 0:
                ...
                channel->inr_no =
                    0x20 +
                    14;  // 从片8259a上倒数第二的中断引脚,温盘,也就是ide0通道的的中断向量号
                break;
            case 1:
                channel->port_base = 0x170;  // ide1通道的起始端口号是0x170
                channel->inr_no =
                    0x20 +
                    15;  // 从8259A上的最后一个中断引脚,我们用来响应ide1通道上的硬盘中断
                break;
        }

        channel->expect_intr = false;  // 未向硬盘写入指令时不期待硬盘的中断
        lock_init(&channel->c_lock);

        /* 初始化为0,目的是向硬盘控制器请求数据后,硬盘驱动sema_down此信号量会阻塞线程,
        直到硬盘完成后通过发中断,由中断处理程序将此信号量sema_up,唤醒线程. */
        sema_init(&channel->disk_down, 0);

        register_hsndler(channel->inr_no, intr_hd_handler);
