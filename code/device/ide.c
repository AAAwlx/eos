...
static void read_from_sector(struct disk* hd, void* buf, uint8_t sec_cnt) {
    ...
    inw(reg_data(hd->my_channel), buf, size_in_byte / 2);
}
...
static void write2sector(struct disk* hd, void* buf, uint8_t sec_cnt) {
    ...
    outw(reg_data(hd->my_channel), buf, size_in_byte / 2);
}
...
/* 打印分区信息 */
static bool partition_info(struct list_node* pelem, int arg UNUSED) {
...
void ide_init() {
    ...
    直到硬盘完成后通过发中断,由中断处理程序将此信号量sema_up,唤醒线程. */
        ...
        register_hsndler(channel->irq_no, intr_hd_handler);
