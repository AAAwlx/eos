...
static int32_t copy_pcb_vaddrbitmap_stack0(struct task_pcb *child_thread, struct task_pcb *parent_thread)
{
    ...
    memcpy(vaddr_btmp, child_thread->userprog_vaddar.vaddr_bitmap.bits, bitmap_pg_cnt * PG_SIZE);
    child_thread->userprog_vaddar.vaddr_bitmap.bits = vaddr_btmp;
    /* 调试用 */
    return 0;
}
...
{
    uint8_t *vaddr_btmp = parent_thread->userprog_vaddar.vaddr_bitmap.bits;
    uint32_t btmp_bytes_len = parent_thread->userprog_vaddar.vaddr_bitmap.bitmap_len;
    uint32_t vaddr_start = parent_thread->userprog_vaddar.vaddr_start;
    uint32_t idx_byte = 0;
    uint32_t idx_bit = 0;
